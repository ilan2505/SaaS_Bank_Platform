...
- Where something read oddly, wasn't how I would have named or structured it, or seemed like more (or less) than the task needed, I asked for it to be changed — e.g. requesting the edit/revalidate workflow be collapsed into one "Save" action instead of two buttons, moving the summary cards, dropping the pie-chart idea after understanding why a stacked bar reads better for close values, and excluding invalid categories from the analytics chart. Nothing shipped that I couldn't explain or didn't agree with.
